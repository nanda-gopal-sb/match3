    ...
    for (int y = 0; y < BOARD_SIZE; y++)
    {
        for (int x = 0; x < BOARD_SIZE; x++)
        {
            ...
            DrawTextEx(gameFont, TextFormat("%c", board[x][y]),
                       (Vector2){rect.x + 12, rect.y + 8}, 35, 1,
                       matches[x][y] ? GREEN : WHITE);
        }
    }
    if (selected.x >= 0)
    {
        DrawRectangleLinesEx((Rectangle){(4 * TILE_SIZE) + (selected.x * TILE_SIZE),
                                         (4 * TILE_SIZE) + (selected.y * TILE_SIZE),
                                         TILE_SIZE, TILE_SIZE},
                             2, YELLOW);
    }
    DrawTextEx(scoreFont, TextFormat("Score : %d", score), (Vector2){20, 20}, 35,
               1, YELLOW);
}
